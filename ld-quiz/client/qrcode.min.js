...
// Custom toCanvas helper for the quiz component
qrcode.toCanvas = function(canvas, text, options) {
  options = options || {};
  let errorCorrectionLevel = options.errorCorrectionLevel || 'Q';
  const levelMap = { 0: 'L', 1: 'M', 2: 'Q', 3: 'H' };
  if (typeof errorCorrectionLevel === 'number') {
    errorCorrectionLevel = levelMap[errorCorrectionLevel] || 'Q';
  }
  let typeNumber = options.typeNumber;
  if (!typeNumber) {
    for (let tn = 1; tn <= 40; tn++) {
      const qr = qrcode(tn, errorCorrectionLevel);
      qr.addData(text);
      try {
        qr.make();
        typeNumber = tn;
  ...
  const marginModules = typeof options.margin === 'number' ? options.margin : 4;
  const moduleSize = options.width ? Math.floor(options.width / (moduleCount + 2 * marginModules)) : 4;
  const codeSize = moduleSize * moduleCount;
  const size = codeSize + 2 * moduleSize * marginModules;
  canvas.width = size;
  canvas.height = size;
  ...
  const offset = moduleSize * marginModules;
